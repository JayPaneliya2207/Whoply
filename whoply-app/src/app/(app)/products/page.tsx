...
export default function ProductsPage() {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex gap-2">
                <button onClick={() => setCatPicker(true)} className="wp-btn wp-btn-ghost flex-1 justify-between">
                    <span className="flex items-center gap-2 truncate">
                        {activeCat ? <><span>{catEmoji(activeCat.name)}</span> {activeCat.name}</> : <><Boxes size={15} /> {t('allCategories')}</>}
                    </span>
                    <ChevronDown size={16} />
                </button>
            </div>
            ...
            {!isLoading && (data || []).length === 0 && <p className="text-sm wp-card p-6 text-center" style={{ color: 'var(--text-muted)' }}>{t('noProductsAdd')}</p>}
            ...
            <Modal open={prodModal} onClose={() => setProdModal(false)} title={editing ? t('editProductTitle') : t('addProduct')}
                footer={<button className="wp-btn wp-btn-primary w-full" disabled={saveProduct.isPending || !form.name || !form.sku} onClick={() => saveProduct.mutate()}>{editing ? t('save') : t('addProduct')}</button>}>
                <Field label={t('productNameLabel')}><input className="wp-input" value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="e.g. Silk Saree" /></Field>
                <div className="grid grid-cols-2 gap-3">
                    <Field label={t('category')}><select className="wp-input" value={form.categoryId} onChange={(e) => set('categoryId', e.target.value)}><option value="">—</option>{(cats || []).map((c: any) => <option key={c._id} value={c._id}>{c.name}</option>)}</select></Field>
                    <Field label={t('skuLabel')}><input className="wp-input" value={form.sku} onChange={(e) => set('sku', e.target.value)} placeholder="SKU code" /></Field>
                </div>
                <Field label={t('barcodeLabel')}>
                    <div className="flex gap-2">
                        <input className="wp-input" value={form.barcode} onChange={(e) => set('barcode', e.target.value)} placeholder={t('scanOrType')} inputMode="numeric" />
                        <ScanButton onScan={(code) => set('barcode', code)} label="" />
                    </div>
                </Field>
                <div className="grid grid-cols-2 gap-3">
                    <Field label={t('unitLabel')}><input className="wp-input" value={form.unit} onChange={(e) => set('unit', e.target.value)} placeholder="pcs / kg / box" /></Field>
                    <Field label={t('hsnCode')}><input className="wp-input" value={form.hsn} onChange={(e) => set('hsn', e.target.value)} placeholder="e.g. 6109 (tax code)" /></Field>
                </div>
                <div className="grid grid-cols-3 gap-3">
                    <Field label={t('costRs')}><input className="wp-input tabular" type="number" value={form.costPrice} onChange={(e) => set('costPrice', e.target.value)} /></Field>
            ...
            {/* Category modal */}
            <Modal open={catModal} onClose={() => setCatModal(false)} title={editingCat ? t('editCategory') : t('addCategory')}
                footer={<button className="wp-btn wp-btn-primary w-full" disabled={saveCat.isPending || !catName} onClick={() => saveCat.mutate()}>{editingCat ? t('save') : t('addCategory')}</button>}>
                <Field label={t('categoryNameLabel')}><input className="wp-input" value={catName} onChange={(e) => setCatName(e.target.value)} placeholder="e.g. Groceries" autoFocus /></Field>
            </Modal>
                    ...
                    {(cats || []).map((c: any) => (
                        <div key={c._id} className="wp-card p-3 text-center relative" style={catFilter === c._id ? { borderColor: 'var(--brand-600)', boxShadow: '0 0 0 1px var(--brand-600)' } : {}}>
                            ...
                            <button onClick={() => { setCatFilter(c._id); setCatPicker(false); }} className="w-full">
                                ...
                                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{c.productCount ?? 0} {t('items')}</p>
                            </button>
                        </div>
                    ))}
                </div>
            ...
            <ConfirmDialog open={!!del} onClose={() => setDel(null)} onConfirm={() => doDelete.mutate()} loading={doDelete.isPending} title={t('deleteProductTitle')} message={`Remove “${del?.name}” from your catalog?`} />
        </div>
    );
}
