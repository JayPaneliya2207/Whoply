...
import { Building2, IndianRupee, Check, MessageCircle, Plus, Pencil, Trash2 } from 'lucide-react';
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h1 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>Dealers</h1>
                <button className="wp-btn wp-btn-primary" onClick={openNew}><Plus size={16} /> Add Dealer</button>
            </div>
                ...
                {(data || []).map((d: any) => (
                    <motion.div key={d._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="wp-card wp-card-hover p-4">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="wp-chip" style={tierTone[d.tier]}>{groupLabel[d.tier]}</span>
                        </div>
                        <div className="mt-3 flex items-center justify-between">
                            ...
                            <div className="flex gap-1.5">
                                ...
                                {d.outstandingBalance > 0 && (
                                    <>
                                        <button className="wp-btn wp-btn-ghost !p-2" title="WhatsApp reminder (stub)" onClick={() => alert(`Payment reminder to ${d.name} — WhatsApp (stubbed in MVP).`)}><MessageCircle size={14} style={{ color: 'var(--success-600)' }} /></button>
            ...
            <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Edit dealer' : 'Add dealer'}
                ...
                <div className="grid grid-cols-3 gap-3">
                    ...
                    <Field label="Credit ₹"><input className="wp-input tabular" type="number" value={form.creditLimit} onChange={(e) => set('creditLimit', e.target.value)} /></Field>
                </div>
                {formErr && <p className="text-sm" style={{ color: 'var(--danger-500)' }}>{formErr}</p>}
            </Modal>

            {/* Collect payment */}
            <Modal open={!!collectFor} onClose={() => setCollectFor(null)} title="Collect payment"
                footer={<button className="wp-btn wp-btn-primary w-full" disabled={collect.isPending || !Number(amount)} onClick={() => collect.mutate()}><Check size={16} /> Confirm</button>}>
                ...
                <Field label="Amount received"><input className="wp-input tabular" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} autoFocus /></Field>
                {collectErr && <p className="text-sm" style={{ color: 'var(--danger-500)' }}>{collectErr}</p>}
            </Modal>

            <ConfirmDialog open={!!del} onClose={() => setDel(null)} onConfirm={() => doDelete.mutate()} loading={doDelete.isPending} title="Remove dealer?" message={`Remove “${del?.name}” from your dealers?`} />
        </div>
    );
}
